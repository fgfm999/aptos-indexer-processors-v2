...
impl ProcessorTrait for CustomProcessor {
    ...
    async fn run_processor(&self) -> anyhow::Result<()> {
        ...
        // NOTE::!!!!! This will also use the config.starting_version, we can : either update the config file or create a new config
        //    to avoid fetch from version 0, comment for now
